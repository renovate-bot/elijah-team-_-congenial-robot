...
import tripleo.elijah_durable_congenial.stages.deduce.post_bytecode.DeduceElement3_Type;
import tripleo.elijah_durable_congenial.stages.deduce.tastic.FCA_Stop;
...
public class Implement_construct {
	...
	private void _implement_construct_type(final @Nullable Constructable co,
		...
		pte.setResolvedElement(best, new GG_ResolveEvent() {
			final String id="Implement_construct::_implement_construct_type";});
		// set FunctionInvocation with pte args
		{
			...
			// TODO also check arguments
			{
				...
				@NotNull FunctionInvocation fi = deduceTypes2.newFunctionInvocation(cc, pte, clsinv, deduceTypes2.getPhase());
...
//			assert attached != null; // TODO will fail when empty variable expression
	...
	private void implement_construct_type(final @Nullable Constructable co,
		...
		if (co != null) {
			...
			classInvocation.resolvePromise().then(co::resolveTypeToClass);
		}
		...
		if (classInvocation != null) {
			if (classInvocation.getConstructorName() != null) {
				...
				gen.getResultPromise().then(new DoneCallback<EvaConstructor>() {
					@Override
					public void onDone(final EvaConstructor gc) {
						classInvocation.resolveDeferred().then(result -> {
							...
							var ccc = deduceTypes2.module.getCompilation();
							try {
								ccc.getCompilationEnclosure().getMalBulge().getEnv().re("(trigger)");
	...
	public class ICH {
		...
		ClassInvocation getClassInvocation(final @Nullable String constructorName,
			...
			if (aGenType != null && aGenType.getCi() != null) {
				...
				clsinv = deduceTypes2.getPhase().registerClassInvocation(clsinv2);
			}
			return clsinv;
		}
